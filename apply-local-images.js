...
const artifactDir = "C:\\Users\\HP\\.gemini\\antigravity\\brain\\7214faf2-8789-4b10-8d06-2702df0f823d";
const publicDir = path.join(__dirname, "public", "products");

// Find all successful generated images in artifact dir
...
const imageMap = {
  ...
  nuts: ["/products/nuts.png"],
  "cold drinks": ["/products/juice.png"]
};

// Copy them
files.forEach(f => {
  if (f.endsWith(".png")) {
    if (f.startsWith("mango_")) {
      fs.copyFileSync(path.join(artifactDir, f), path.join(publicDir, f));
      imageMap.mango.push("/products/" + f);
    }
    if (f.startsWith("dates_")) {
      fs.copyFileSync(path.join(artifactDir, f), path.join(publicDir, f));
      imageMap.dates.push("/products/" + f);
    }
    if (f.startsWith("ghee_")) {
      fs.copyFileSync(path.join(artifactDir, f), path.join(publicDir, f));
      imageMap.ghee.push("/products/" + f);
    }
  }
});

// Fallbacks if arrays are empty (from previous generation)
if (imageMap.mango.length === 0) imageMap.mango = ["/products/mango.png"];
if (imageMap.dates.length === 0) imageMap.dates = ["/products/dates.png"];
if (imageMap.ghee.length === 0) imageMap.ghee = ["/products/ghee.png"];

console.log("Images available:", JSON.stringify(imageMap, null, 2));
...
categories.forEach((cat, catIndex) => {
  ...
  cat.baseProducts.forEach((prodName, i) => {
    ...
    // Round robin the available images
...
console.log("Updated MOCK_PRODUCTS using local Gemini images!");
