    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-xs font-semibold text-[#475569]">Loading financial data...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-6 max-w-[1400px] text-[#0F172A] font-sans">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white border border-[#EEF2F7] rounded-md p-5 space-y-3 shadow-sm border-l-4 border-l-emerald-500">
          ...
          <p className="text-2xl font-black text-[#0F172A]">{formatCurrency(totalRevenue)}</p>
        </div>

        <div className="bg-white border border-[#EEF2F7] rounded-md p-5 space-y-3 shadow-sm border-l-4 border-l-amber-500">
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-bold text-[#94A3B8] uppercase tracking-wider">Outstanding (Pending)</span>
            <div className="w-8 h-8 rounded-md bg-amber-50 flex items-center justify-center">
              <AlertCircle className="w-4 h-4 text-amber-600" />
            </div>
          </div>
          <p className="text-2xl font-black text-[#0F172A]">{formatCurrency(outstandingAmount)}</p>
        </div>

        <div className="bg-white border border-[#EEF2F7] rounded-md p-5 space-y-3 shadow-sm border-l-4 border-l-slate-400">
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-bold text-[#94A3B8] uppercase tracking-wider">Refunds Processed</span>
            <div className="w-8 h-8 rounded-md bg-slate-100 flex items-center justify-center">
              <RefreshCw className="w-4 h-4 text-slate-600" />
            </div>
          </div>
          <p className="text-2xl font-black text-[#0F172A]">{formatCurrency(refundedAmount)}</p>
        </div>
      </div>
      ...
      <div className="bg-white border border-[#EEF2F7] rounded-md p-4 flex flex-col lg:flex-row gap-4 items-center justify-between shadow-sm">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto justify-start lg:justify-end">
          ...
          <button onClick={() => toggleSort("date")} className={`flex items-center gap-1 text-xs font-bold px-3.5 py-2 rounded-md border transition-all cursor-pointer ${sortBy === "date" ? "bg-emerald-600 text-white border-emerald-600" : "bg-white text-[#475569] border-[#EEF2F7]"}`}>
            Date <ArrowUpDown className="w-3 h-3 ml-0.5" />
          </button>
          <button onClick={() => toggleSort("amount")} className={`flex items-center gap-1 text-xs font-bold px-3.5 py-2 rounded-md border transition-all cursor-pointer ${sortBy === "amount" ? "bg-emerald-600 text-white border-emerald-600" : "bg-white text-[#475569] border-[#EEF2F7]"}`}>
            Amount <ArrowUpDown className="w-3 h-3 ml-0.5" />
          </button>
        </div>
      </div>
            ...
            <table className="w-full border-collapse text-left">
              <thead>
                <tr className="bg-[#F8FAFC] border-b border-[#EEF2F7]">
                  <th className="px-6 py-4 text-[10px] font-black text-[#94A3B8] uppercase tracking-wider">Transaction Details</th>
                  <th className="px-6 py-4 text-[10px] font-black text-[#94A3B8] uppercase tracking-wider">Customer</th>
                  <th className="px-6 py-4 text-[10px] font-black text-[#94A3B8] uppercase tracking-wider">Amount</th>
                    ...
                    <tr key={order.id} className="hover:bg-[#F8FAFC]/80 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          ...
                          <span className="text-[10px] font-medium text-[#64748B] mt-0.5 font-mono">
                      ...
                      <td className="px-6 py-4">
                        <span className={`text-[10px] font-black uppercase px-2.5 py-1 rounded-md border inline-flex items-center gap-1.5 shadow-sm ${
                          isPaid ? "bg-emerald-50 text-emerald-700 border-emerald-200/50" : 
                          isPending ? "bg-amber-50 text-amber-700 border-amber-200/50" : 
                          isRefunded ? "bg-slate-50 text-slate-700 border-slate-200/50" : 
                          "bg-rose-50 text-rose-700 border-rose-200/50"
                        }`}>
                          <span className={`w-1.5 h-1.5 rounded-sm ${
                            isPaid ? "bg-emerald-500" : 
                            isPending ? "bg-amber-500 animate-pulse" : 
                            isRefunded ? "bg-slate-500" : "bg-rose-500"
                          }`} />
                            ...
                            <>
                              ...
                              {activeMenu === order.id && (
                                <>
                                  ...
                                  <div className="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg bg-white border border-[#EEF2F7] ring-1 ring-black ring-opacity-5 z-20 focus:outline-none animate-fade-in py-1">
                                    <div className="px-3 py-2 text-[10px] font-black uppercase tracking-wider text-[#94A3B8] border-b border-[#EEF2F7] bg-[#F8FAFC]">
                                      Change Payment Status
                                    </div>
                                    {!isPaid && (
                                      <button
                                        onClick={() => handleUpdatePaymentStatus(order.id, "paid")}
                                        className="w-full text-left block px-4 py-2.5 text-xs font-bold text-emerald-600 hover:bg-emerald-50 transition-colors"
                                      >
                                        Mark as Paid
                                      </button>
                                    )}
                                    {!isPending && (
                                      <button
                                        ...
                                        className="w-full text-left block px-4 py-2.5 text-xs font-bold text-amber-600 hover:bg-amber-50 transition-colors"
                                      >
                                        Mark as Pending
                                      </button>
                                    )}
                                    {!isRefunded && isPaid && (
                                      <button
                                        onClick={() => {
                                          if(window.confirm("Are you sure you want to mark this as refunded? This does NOT automatically refund via SSLCommerz, it only updates the database.")) {
                                            handleUpdatePaymentStatus(order.id, "refunded");
                                          }
                                        }}
                                        className="w-full text-left block px-4 py-2.5 text-xs font-bold text-slate-600 hover:bg-slate-50 transition-colors"
                                      >
                                        Mark as Refunded
                                      </button>
                                    )}
                                    {!isFailed && (
                                      <button
                                        ...
                                        className="w-full text-left block px-4 py-2.5 text-xs font-bold text-rose-600 hover:bg-rose-50 transition-colors"
                                      >
                                        Mark as Failed
                                      </button>
                                    )}
