            ...
            <table className="w-full border-collapse text-left">
              <thead>
                <tr className="bg-[#F8FAFC] border-b border-[#EEF2F7]">
                  ...
                  <th className="px-6 py-4"><div className="h-3 w-48 bg-slate-200 rounded"></div></th>
              ...
              <tbody className="divide-y divide-[#EEF2F7]">
                {[...Array(5)].map((_, i) => (
                  <tr key={i}>
                    ...
                    <td className="px-6 py-4">
                      <div className="h-16 w-full max-w-sm bg-slate-200 rounded-md"></div>
                    </td>
                    <td className="px-6 py-4">
    ...
    <div className="flex flex-col gap-6 max-w-[1400px] text-[#0F172A] font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="font-serif-heading text-xl sm:text-2xl font-black text-[#0F172A] flex items-center gap-2">
            <MessageSquareQuote className="w-5 sm:w-6 h-5 sm:h-6 text-amber-500" />
            Product Reviews
          </h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white border border-[#EEF2F7] rounded-md p-5 space-y-3 shadow-sm border-l-4 border-l-blue-500">
          <div className="flex items-center justify-between">
            <span className="text-xs lg:text-[10px] font-bold text-[#94A3B8] uppercase tracking-wider">Total Reviews</span>
            <div className="w-8 h-8 rounded-md bg-blue-50 flex items-center justify-center">
              <MessageSquareQuote className="w-4 h-4 text-blue-600" />
            </div>
          </div>
          <p className="text-xl sm:text-2xl font-black text-[#0F172A]">{totalReviews}</p>
        </div>

        <div className="bg-white border border-[#EEF2F7] rounded-md p-5 space-y-3 shadow-sm border-l-4 border-l-amber-500">
          <div className="flex items-center justify-between">
            <span className="text-xs lg:text-[10px] font-bold text-[#94A3B8] uppercase tracking-wider">Store Average</span>
            <div className="w-8 h-8 rounded-md bg-amber-50 flex items-center justify-center">
              <Star className="w-4 h-4 fill-amber-500 text-amber-500" />
            </div>
          </div>
          <p className="text-xl sm:text-2xl font-black text-[#0F172A]">{avgRating} <span className="text-sm font-semibold text-[#94A3B8]">/ 5.0</span></p>
        </div>

        <div className="bg-white border border-[#EEF2F7] rounded-md p-5 space-y-3 shadow-sm border-l-4 border-l-slate-400">
          <div className="flex items-center justify-between">
            <span className="text-xs lg:text-[10px] font-bold text-[#94A3B8] uppercase tracking-wider">Hidden / Rejected</span>
            <div className="w-8 h-8 rounded-md bg-slate-100 flex items-center justify-center">
              <ShieldAlert className="w-4 h-4 text-slate-600" />
            </div>
          </div>
          <p className="text-xl sm:text-2xl font-black text-[#0F172A]">{hiddenCount}</p>
        </div>
      </div>
      ...
      <div className="bg-white border border-[#EEF2F7] rounded-md p-3 sm:p-4 flex flex-col lg:flex-row gap-3 sm:gap-4 items-stretch lg:items-center justify-between shadow-sm">
        ...
        <div className="flex flex-wrap items-center gap-2 w-full lg:w-auto">
          ...
          <div className="flex items-center gap-1.5 bg-[#F8FAFC] border border-[#EEF2F7] px-3 py-2.5 sm:px-3.5 sm:py-2 rounded-md transition-all focus-within:border- focus-within:ring-4 focus-within:ring-amber-500/10">
          ...
          <button onClick={() => toggleSort("rating")} className={`flex items-center gap-1 text-sm sm:text-xs font-bold px-3.5 py-2.5 sm:py-2 rounded-md border transition-all cursor-pointer flex-1 sm:flex-none justify-center ${sortBy === "rating" ? "bg-amber-500 text-slate-900 border-amber-500" : "bg-white text-[#475569] border-[#EEF2F7]"}`}>
            Rating <ArrowUpDown className="w-3.5 sm:w-3 h-3.5 sm:h-3 ml-0.5" />
          </button>
        </div>
      </div>
...
<>
            ...
            <div className="hidden lg:block overflow-x-auto">
              <table className="w-full border-collapse text-left">
                <thead>
                  <tr className="bg-[#F8FAFC] border-b border-[#EEF2F7]">
                    <th className="px-6 py-4 text-xs lg:text-[10px] font-black text-[#94A3B8] uppercase tracking-wider">Product & Rating</th>
                    <th className="px-6 py-4 text-xs lg:text-[10px] font-black text-[#94A3B8] uppercase tracking-wider">Customer Comment</th>
                    <th className="px-6 py-4 text-xs lg:text-[10px] font-black text-[#94A3B8] uppercase tracking-wider">Reviewer</th>
                    <th className="px-6 py-4 text-xs lg:text-[10px] font-black text-[#94A3B8] uppercase tracking-wider">Visibility</th>
                      ...
                      <tr key={review.id} className={`hover:bg-[#F8FAFC]/80 transition-colors ${!review.is_approved ? 'bg-slate-50/50' : ''}`}>
                        ...
                        <td className="px-6 py-4 align-top">
                          <div className="flex flex-col">
                            ...
                            <span className="text-xs lg:text-[10px] text-[#94A3B8]">{review.profile?.email}</span>
                            <span className="text-xs lg:text-[10px] text-[#94A3B8] mt-2">
            ...
            <div className="lg:hidden divide-y divide-[#EEF2F7]">
              {filteredReviews.map((review) => {
                return (
                  <div key={review.id} className={`p-4 space-y-3 ${!review.is_approved ? 'bg-slate-50/50' : ''}`}>
                    {/* Product & Stars */}
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-md border border-[#EEF2F7] bg-white overflow-hidden shrink-0">
                        {review.product?.images?.[0] ? (
                    ...
                    <div className="flex flex-col">
                      <span className="text-xs font-bold text-[#0F172A]">{review.profile?.full_name || "Anonymous User"}</span>
                      <span className="text-xs text-[#94A3B8]">{review.profile?.email}</span>
                      ...
                      <div className="flex items-center gap-2.5">
                        {updating === review.id ? (
                          ...
                          <button
                            onClick={() => handleToggleStatus(review)}
                            title={review.is_approved ? "Hide Review" : "Approve & Show Review"}
                            className={`p-2.5 rounded-md border transition-all shadow-sm cursor-pointer ${
                              review.is_approved 
                                ? "border-slate-200 bg-white text-slate-600 hover:bg-slate-100" 
                                : "border-emerald-200 bg-emerald-50 text-emerald-600 hover:bg-emerald-100"
                            }`}
