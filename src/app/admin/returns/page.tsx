    ...
    <div className="flex flex-col gap-6 max-w-[1400px] text-[#0F172A] font-sans">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="font-serif-heading text-2xl font-black text-[#0F172A] flex items-center gap-2">
            <CornerDownLeft className="w-6 h-6 text-indigo-500" />
            Return & Refund Requests
          </h2>
          <p className="text-xs text-[#475569] mt-1">Manage customer return and refund requests.</p>
        </div>
      </div>
      ...
      <div className="bg-white border border-[#EEF2F7] rounded-md p-4 flex gap-4 items-center shadow-sm">
        <div className="relative flex-1 max-w-md">
          ...
          <input type="text" placeholder="Search by ID, customer..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
            className="w-full pl-10 pr-4 py-2 rounded-md border border-[#EEF2F7] text-xs font-semibold focus:outline-none focus:border-indigo-500" />
        </div>
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
          className="px-3 py-2 rounded-md border border-[#EEF2F7] text-xs font-bold focus:outline-none cursor-pointer bg-white">
        ...
        {filtered.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-[#F8FAFC] border-b border-[#EEF2F7]">
                  <th className="px-5 py-3 text-[10px] font-black uppercase text-[#94A3B8]">Customer</th>
                  <th className="px-5 py-3 text-[10px] font-black uppercase text-[#94A3B8]">Order</th>
                  <th className="px-5 py-3 text-[10px] font-black uppercase text-[#94A3B8]">Reason</th>
                  <th className="px-5 py-3 text-[10px] font-black uppercase text-[#94A3B8]">Date</th>
                  <th className="px-5 py-3 text-[10px] font-black uppercase text-[#94A3B8]">Status</th>
                  <th className="px-5 py-3 text-[10px] font-black uppercase text-[#94A3B8] text-right">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#EEF2F7]">
                {filtered.map(r => (
                  <tr key={r.id} className="hover:bg-[#F8FAFC]/60">
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-[#64748B]">{r.profile?.email}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-5 py-3">
                    ...
                    <td className="px-5 py-3">
                      <span className={`text-[10px] font-bold uppercase px-2.5 py-1 rounded-md border inline-flex items-center gap-1 ${STATUS_COLORS[r.status] || ""}`}>
                      ...
                      <div className="flex items-center justify-end gap-1.5">
                        {r.status === "pending" && (
                          <>
                            <button onClick={() => handleStatus(r.id, "approved")} disabled={updating === r.id}
                              className="px-3 py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-700 text-[10px] font-bold rounded-md border border-blue-200 transition-all cursor-pointer disabled:opacity-50 flex items-center gap-1">
                              <CheckCircle2 className="w-3 h-3" /> Approve
                            </button>
                            <button onClick={() => handleStatus(r.id, "rejected")} disabled={updating === r.id}
                              className="px-3 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-700 text-[10px] font-bold rounded-md border border-rose-200 transition-all cursor-pointer disabled:opacity-50 flex items-center gap-1">
                        ...
                        {r.status === "approved" && (
                          <button onClick={() => handleStatus(r.id, "refunded")} disabled={updating === r.id}
                            className="px-3 py-1.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 text-[10px] font-bold rounded-md border border-emerald-200 transition-all cursor-pointer disabled:opacity-50 flex items-center gap-1">
