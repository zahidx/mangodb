    ...
    <div className="flex flex-col gap-6 max-w-[1400px] text-[#0F172A] font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <button
          ...
          className="px-4.5 py-2.5 bg-slate-900 hover:bg-fuchsia-600 text-white font-black text-xs uppercase tracking-wider rounded-md hover:scale-[1.02] active:scale-[0.98] transition-all duration-300 flex items-center gap-2 shrink-0 cursor-pointer shadow-md"
        >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg bg-white border border-[#EEF2F7] rounded-md shadow-2xl overflow-hidden z-10 animate-fade-in text-left flex flex-col">
            <div className="p-6 border-b border-[#EEF2F7] flex items-center justify-between bg-[#F8FAFC] shrink-0">
              <div>
                <h3 className="font-serif-heading text-lg font-bold text-[#0F172A]">{isAddModalOpen ? "New Content Block" : "Edit Content Block"}</h3>
                <p className="text-[10px] text-[#94A3B8]">Configure visual elements for the storefront.</p>
              </div>
            ...
            <form onSubmit={isAddModalOpen ? handleAddBanner : handleUpdateBanner} className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1.5 col-span-2">
                  <label className="text-[10px] font-black uppercase text-[#475569]">Headline / Title *</label>
                  <input type="text" required value={formData.title} onChange={(e) => setFormData({...formData, title: e.target.value})} placeholder="e.g. Summer Mango Festival" className="w-full px-3 py-2.5 rounded-md border border-[#EEF2F7] text-xs font-bold focus:outline-none focus:border-fuchsia-500" />
                </div>
                
                <div className="space-y-1.5 col-span-2">
                  <label className="text-[10px] font-black uppercase text-[#475569]">Subtitle</label>
                  <input type="text" value={formData.subtitle} onChange={(e) => setFormData({...formData, subtitle: e.target.value})} placeholder="e.g. Get 20% off all Fazli mangoes" className="w-full px-3 py-2.5 rounded-md border border-[#EEF2F7] text-xs font-semibold focus:outline-none focus:border-fuchsia-500" />
                </div>

                <div className="space-y-1.5 col-span-2">
                  <label className="text-[10px] font-black uppercase text-[#475569]">Image URL *</label>
                  <div className="flex gap-2">
                    <div className="w-10 h-10 rounded-md border border-[#EEF2F7] bg-slate-50 flex items-center justify-center shrink-0 overflow-hidden">
                      {formData.image_url ? (
                    ...
                    <input type="url" required value={formData.image_url} onChange={(e) => setFormData({...formData, image_url: e.target.value})} placeholder="https://..." className="w-full px-3 py-2.5 rounded-md border border-[#EEF2F7] text-xs font-semibold focus:outline-none focus:border-fuchsia-500 flex-grow" />
                  </div>
                </div>

                <div className="space-y-1.5 col-span-2">
                  <label className="text-[10px] font-black uppercase text-[#475569]">Target Link URL (Optional)</label>
                  <input type="url" value={formData.link_url} onChange={(e) => setFormData({...formData, link_url: e.target.value})} placeholder="/collections/summer-sale" className="w-full px-3 py-2.5 rounded-md border border-[#EEF2F7] text-xs font-semibold focus:outline-none focus:border-fuchsia-500" />
                </div>

                <div className="space-y-1.5">
                  <label className="text-[10px] font-black uppercase text-[#475569]">Placement Area</label>
                  <select value={formData.position} onChange={(e) => setFormData({...formData, position: e.target.value as any})} className="w-full px-3 py-2.5 rounded-md border border-[#EEF2F7] text-xs font-bold focus:outline-none focus:border-fuchsia-500 cursor-pointer">
                ...
                <div className="space-y-1.5">
                  ...
                  <input type="number" value={formData.sort_order} onChange={(e) => setFormData({...formData, sort_order: e.target.value})} placeholder="0" className="w-full px-3 py-2.5 rounded-md border border-[#EEF2F7] text-xs font-semibold focus:outline-none focus:border-fuchsia-500" />
                </div>
              </div>

              <label className="flex items-center gap-2 text-xs font-bold text-[#0F172A] cursor-pointer mt-2 pt-2 border-t border-[#EEF2F7]">
                <input type="checkbox" checked={formData.is_active} onChange={(e) => setFormData({...formData, is_active: e.target.checked})} className="rounded border-slate-300 text-fuchsia-600 focus:ring-fuchsia-500" />
                Publish this block (Visible on storefront)
              </label>
              ...
              <div className="pt-2 flex justify-end gap-3 shrink-0">
                ...
                <button type="submit" className="px-5 py-2 bg-fuchsia-600 hover:bg-fuchsia-700 text-white font-extrabold text-xs rounded-md shadow-sm hover:scale-[1.02] active:scale-[0.98] transition-all cursor-pointer">
