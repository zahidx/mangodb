    ...
    <div className="flex flex-col gap-6 max-w-[1400px] text-[#0F172A] font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="font-serif-heading text-xl sm:text-2xl font-black text-[#0F172A] flex items-center gap-2">
            <Ticket className="w-5 sm:w-6 h-5 sm:h-6 text-pink-600" />
            Coupons & Promotions
          </h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white border border-[#EEF2F7] rounded-md p-5 space-y-3 shadow-sm border-l-4 border-l-pink-500">
          <div className="flex items-center justify-between">
            <span className="text-xs lg:text-[10px] font-bold text-[#94A3B8] uppercase tracking-wider">Active Campaigns</span>
            <div className="w-8 h-8 rounded-md bg-pink-50 flex items-center justify-center">
              <CheckCircle2 className="w-4 h-4 text-pink-600" />
            </div>
          </div>
          <p className="text-xl sm:text-2xl font-black text-[#0F172A]">{activeCount}</p>
        </div>

        <div className="bg-white border border-[#EEF2F7] rounded-md p-5 space-y-3 shadow-sm border-l-4 border-l-purple-500">
          <div className="flex items-center justify-between">
            <span className="text-xs lg:text-[10px] font-bold text-[#94A3B8] uppercase tracking-wider">Total Usages</span>
            <div className="w-8 h-8 rounded-md bg-purple-50 flex items-center justify-center">
              <Percent className="w-4 h-4 text-purple-600" />
            </div>
          </div>
          <p className="text-xl sm:text-2xl font-black text-[#0F172A]">{totalUsages}</p>
        </div>

        <div className="bg-white border border-[#EEF2F7] rounded-md p-5 space-y-3 shadow-sm border-l-4 border-l-slate-400">
          <div className="flex items-center justify-between">
            <span className="text-xs lg:text-[10px] font-bold text-[#94A3B8] uppercase tracking-wider">Expired Codes</span>
            <div className="w-8 h-8 rounded-md bg-slate-100 flex items-center justify-center">
              <CalendarX className="w-4 h-4 text-slate-600" />
            </div>
          </div>
          <p className="text-xl sm:text-2xl font-black text-[#0F172A]">{expiredCount}</p>
        </div>
      </div>
      ...
      <div className="bg-white border border-[#EEF2F7] rounded-md p-3 sm:p-4 flex flex-col lg:flex-row gap-3 sm:gap-4 items-stretch lg:items-center justify-between shadow-sm">
        <div className="relative w-full lg:w-96">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 sm:py-2.5 rounded-md border border-[#EEF2F7] bg-slate-50/50 text-sm sm:text-xs font-semibold text-[#0F172A] placeholder-[#94A3B8] focus:bg-white focus:outline-none focus:border-pink-500 focus:ring-4 focus:ring-pink-500/10 transition-all uppercase"
          />
        </div>

        <div className="flex flex-wrap items-center gap-2 w-full lg:w-auto">
          <div className="flex items-center gap-1.5 bg-[#F8FAFC] border border-[#EEF2F7] px-3 py-2.5 sm:px-3.5 sm:py-2 rounded-md transition-all focus-within:border-pink-500 focus-within:ring-4 focus-within:ring-pink-500/10">
          ...
          <button onClick={() => toggleSort("date")} className={`flex items-center gap-1 text-sm sm:text-xs font-bold px-3.5 py-2.5 sm:py-2 rounded-md border transition-all cursor-pointer flex-1 sm:flex-none justify-center ${sortBy === "date" ? "bg-pink-600 text-white border-pink-600" : "bg-white text-[#475569] border-[#EEF2F7]"}`}>
            Date <ArrowUpDown className="w-3.5 sm:w-3 h-3.5 sm:h-3 ml-0.5" />
          </button>
          <button onClick={() => toggleSort("usage")} className={`flex items-center gap-1 text-sm sm:text-xs font-bold px-3.5 py-2.5 sm:py-2 rounded-md border transition-all cursor-pointer flex-1 sm:flex-none justify-center ${sortBy === "usage" ? "bg-pink-600 text-white border-pink-600" : "bg-white text-[#475569] border-[#EEF2F7]"}`}>
            Usage <ArrowUpDown className="w-3.5 sm:w-3 h-3.5 sm:h-3 ml-0.5" />
          </button>
        </div>
      </div>
...
<>
            ...
            <div className="hidden lg:block overflow-x-auto">
              <table className="w-full border-collapse text-left">
                <thead>
                  <tr className="bg-[#F8FAFC] border-b border-[#EEF2F7]">
                    <th className="px-6 py-4 text-xs lg:text-[10px] font-black text-[#94A3B8] uppercase tracking-wider">Coupon Code</th>
                    <th className="px-6 py-4 text-xs lg:text-[10px] font-black text-[#94A3B8] uppercase tracking-wider">Discount</th>
                    <th className="px-6 py-4 text-xs lg:text-[10px] font-black text-[#94A3B8] uppercase tracking-wider">Usage & Limits</th>
                      ...
                      <tr key={coupon.id} className={`hover:bg-[#F8FAFC]/80 transition-colors ${(expired || !coupon.is_active) ? 'opacity-70' : ''}`}>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex flex-col">
                            ...
                            <span className="text-xs lg:text-[10px] text-[#94A3B8] mt-1">
                        ...
                        <td className="px-6 py-4">
                          <div className="flex flex-col">
                            ...
                            {coupon.min_order_amount > 0 && (
                              <span className="text-xs lg:text-[10px] text-[#64748B] font-medium mt-1">
                                Min Order: ৳{coupon.min_order_amount}
                              </span>
                            )}
                            {coupon.max_discount_amount && (
                              <span className="text-xs lg:text-[10px] text-[#64748B] font-medium">
                        ...
                        <td className="px-6 py-4">
                          <div className="flex flex-col gap-1.5">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-xs lg:text-[10px] font-bold text-[#475569]">
                                {coupon.used_count} {coupon.usage_limit ? `/ ${coupon.usage_limit}` : 'used'}
                              </span>
                            </div>
                            {coupon.expires_at && (
                              <span className={`text-xs lg:text-[10px] font-medium flex items-center gap-1 ${expired ? 'text-rose-600' : 'text-[#64748B]'}`}>
            ...
            <div className="lg:hidden divide-y divide-[#EEF2F7]">
              {filteredCoupons.map((coupon) => {
                ...
                return (
                  <div key={coupon.id} className={`p-4 space-y-3 ${(expired || !coupon.is_active) ? 'opacity-70' : ''}`}>
                    {/* Code & Discount */}
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-black text-[#0F172A] uppercase tracking-wide bg-slate-100 px-2 py-1 rounded-md border border-slate-200">
                        {coupon.code}
                      </span>
                      <span className="text-sm font-bold text-pink-600">
                        {coupon.discount_type === "percentage" ? `${coupon.discount_value}% OFF` : `৳${coupon.discount_value} OFF`}
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-[#64748B] font-medium">
                      {coupon.min_order_amount > 0 && (
                    ...
                    {/* Expiry */}
                    {coupon.expires_at && (
                      <span className={`text-xs font-medium flex items-center gap-1 ${expired ? 'text-rose-600' : 'text-[#64748B]'}`}>
                        <CalendarX className="w-3 h-3" />
                        {expired ? "Expired " : "Ends "}
                        {new Date(coupon.expires_at).toLocaleDateString()}
                      </span>
